...
If you are using Headlamp as a desktop app (Windows, Mac, or Linux), first go Settings/Plugins/Plugin Catalog. Next go to the Plugin Catalog (available from the sidebar when in the home view), then look for the Kubescape plugin from the list, click it to see its details, and then click the install button. After the plugin is installed, use the notification to refresh the UI, or use the app menu Navigation > Reload.
...
A huge thanks to the Kubescape team for building such a powerful tool for security posture management.
